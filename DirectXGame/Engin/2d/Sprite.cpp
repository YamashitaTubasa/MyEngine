...
void Sprite::SpriteCreateGraphicsPipeline(ID3D12Device* device) 
{
	HRESULT result;
	...
	descHeapDesc.NumDescriptors = kMaxSRVCount; // シェーダーリソースビュー1つ
...
void Sprite::SpriteCreate(ID3D12Device* dev, float window_width, float window_height,
	UINT texNumber, XMFLOAT2 anchorpoint, bool isFlipX, bool isFlipY)
{
	...
	// 指定番号の画像が読み込み済みなら
	if (texBuff[this->texNumber]) {
		// テクスチャ情報取得
		D3D12_RESOURCE_DESC resDesc = texBuff[this->texNumber]->GetDesc();
...
void Sprite::SpriteDraw(ID3D12GraphicsCommandList* cmdList_, ID3D12Device* dev) 
{
	// 非表示フラグがtrueなら
	if (isInvisible) {
		// 描画せずに抜ける
...
//
//	// スプライト用パイプライン生成
//	pipelinestate = SpriteCreateGraphicsPipeline(dev);
//
//	// 平行投影行列生成
//	matProjection = XMMatrixOrthographicOffCenterLH(
...
void Sprite::SpriteUpdate(Sprite* sprite) 
{
	// ワールド行列の更新
...
void Sprite::SpriteTransferVertexBuffer(const Sprite* sprite, uint32_t texIndex)
{
	...
	// UV計算
	// 指定番号の画像が読み込み済みなら
	if (texBuff[sprite->texIndex_]) {
		// テクスチャ情報取得
		resDesc = texBuff[sprite->texIndex_]->GetDesc();
